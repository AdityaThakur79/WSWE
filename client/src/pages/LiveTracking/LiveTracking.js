...
const LiveTracking = () => {
  ...
  const createLink = async () => {
    try {
      const { data } = await axios.post("/api/v1/user/createLink", {
        userId: user?._id,
      });
      console.log(data);
      setLink(data?.url);
      ...
      if (navigator.geolocation) {
        navigator.geolocation.watchPosition(
          (position) => {
            ...
            console.log(
              "location has been updated on live tracking",
              latitude,
              longitude
            );
          },
  ...
  return (
    <div className="flex flex-col items-center h-screen gap-4 ">
      ...
      <div className="text-center">
        <div
          className={`p-4 bg-blue-700 text-white text-lg rounded-lg shadow-lg font-semibold border-blue-500 hover:animate-none focus:animate-none ${
      ...
      <button
        className="border-2 p-2 rounded-md text-white font-semibold bg-green-500 hover:bg-green-600"
